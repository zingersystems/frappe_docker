...
- During the current production deployment only, mount the WSL operator's `~/.ssh` directory read-only at `/home/frappe/.ssh`; never copy keys into the workspace, image, or VPS, and remove this exception and mount when deployment access is complete.
